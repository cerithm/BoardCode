...
im = PIL.Image.open(r"C:\Users\Maggie\Pictures\Woody pictures\F1.png")
...
image5 = openImage(r"C:\Users\Maggie\Pictures\Woody pictures\F1.png")
...
holdf = holdsizefilter(holds, 6,62)
...
def Routemake(holds,image,starthold,armlength):
    ...
    R= [Xlist,Ylist]
    dmax=int(1.58*armlength)
    
    while Y>97:
        
        X = X + random.randint(int(-1.6*dmax) ,int(1.6*dmax))
        Y = Y - random.randint(int(1.5*dmax) ,int(1.9*dmax))
...
def Routemake2(holds,image,starthold1,starthold2,armlength):
    
    start1=holds[0][starthold1-1]
    start2=holds[0][starthold2-1]
    X1= start1[0]
    Y1= start1[1]
    X2= start2[0]
    Y2= start2[1]
    ...
    R[0] = R[0] + X2list
    R[1] = R[1] + Y2list
    while Y1>90 or Y2>90:
        xrand=np.arange((-2*dmax),int(2*dmax),1)
        xrand.tolist()
        
        k = len(xrand)
        beta = np.random.beta(9,99,k)
        
        choice=random.choices(xrand,beta)
        print(choice)
        X1 = X1 + choice[0]
              
        
        #X1 = X1 + random.randint(int(-1.6*dmax) ,int(1.6*dmax))
        Y1 = Y1 - random.randint(int(1.5*dmax) ,int(1.9*dmax))
        X2 = X1 + random.randint(int(-1.8*dmax) ,int(1.8*dmax))
        Y2 = Y1 - random.randint(int(-1.9*dmax) ,int(-0.5*dmax))
        print(Y1)
        print(Y2)
        z1=[X1,Y1]
        z2=[X2,Y2]
        holdchoice1=Mindistancetoholds(z1,holds)
        holdchoice2=Mindistancetoholds(z2,holds)       
        R[0] = R[0] + [holds[0][holdchoice1][0]]
        R[1] = R[1] + [holds[0][holdchoice1][1]]
        #R[0] = R[0] + [holds[0][holdchoice2][0]]
        #R[1] = R[1] + [holds[0][holdchoice2][1]]
     
    return R

a = np.array(Routemake2(holdf,image5,49,63,84))
print(a[0])
print(a[1])
print(a[0][2])
...
cv2.imwrite(r"C:\Users\Maggie\Pictures\Woody pictures\routemade2.png", image5)
